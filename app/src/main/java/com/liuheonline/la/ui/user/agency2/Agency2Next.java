...
public class Agency2Next extends BaseMVPActivity<BaseView<Object>,AddMemberPresenter,Object> {
    @BindId(R.id.rb1)
    RadioButton rb1;
    @BindId(R.id.rb2)
    RadioButton rb2;
               ...
               String strname = name.getText().toString();
                ...
                if (strname.trim().length()!=0){//名字不为空
                    if (strselect.trim().length()!=0){//返点不为空
                        presenter.addMember(strname,strselect,agent);
                    }else {
    ...
    protected void setContentView() {
        setContentView(R.layout.activity_agencynext);
    }
