...
public class NumStatisticsInfo extends BaseMVPActivity<BaseView<NumStatisticsEntity>,NumStatisticsPresenter,NumStatisticsEntity> {

    @BindId(R.id.num_info_recycle)
    private RecyclerView recyclerView;

    private BaseQuickAdapter<NumStatisticsEntity.TemaBean,BaseViewHolder> baseQuickAdapter;
    ...
    protected void initData() {
        ...
        baseQuickAdapter = new BaseQuickAdapter<NumStatisticsEntity.TemaBean, BaseViewHolder>(R.layout.item_num_info) {
            @Override
            protected void convert(BaseViewHolder helper, NumStatisticsEntity.TemaBean item) {
                helper.setBackgroundRes(R.id.num_ball_info, LotteryTypeUtil.getStasisticsBallBG(item.getTema_color_id()));
                helper.setText(R.id.num_ball_info,item.getTema()+"");
                helper.setText(R.id.num_index_info,item.getTema_num()+"");
            }
        };
    }
    ...
    public void successed(NumStatisticsEntity numStatisticsEntity) {
        if(numStatisticsEntity != null && numStatisticsEntity.getTema().size() > 0){
            baseQuickAdapter.setNewData(numStatisticsEntity.getTema());
        }
    }
