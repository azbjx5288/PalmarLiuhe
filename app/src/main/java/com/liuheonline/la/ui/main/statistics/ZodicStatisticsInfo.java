...
public class ZodicStatisticsInfo extends BaseMVPActivity<BaseView<ZodiacStatisticsEntity>,ZodiacStatisticsPresenter,ZodiacStatisticsEntity> {
    @BindId(R.id.num_info_recycle)
    private RecyclerView recyclerView;

    private BaseQuickAdapter<ZodiacStatisticsEntity.SingleBean,BaseViewHolder> baseQuickAdapter;
    ...
    protected void initData() {
        baseQuickAdapter = new BaseQuickAdapter<ZodiacStatisticsEntity.SingleBean, BaseViewHolder>(R.layout.item_num_info) {
            @Override
            protected void convert(BaseViewHolder helper, ZodiacStatisticsEntity.SingleBean item) {
                helper.setBackgroundRes(R.id.num_ball_info, LotteryTypeUtil.getZodiac(item.getSingle_attr()));
                helper.setText(R.id.num_index_info,item.getSingle_num()+"");
            }
        };
    }
    ...
    public void successed(ZodiacStatisticsEntity zodiacStatisticsEntity) {
        if(zodiacStatisticsEntity != null && zodiacStatisticsEntity.getSingle().size() > 0){
            baseQuickAdapter.setNewData(zodiacStatisticsEntity.getSingle());
        }
    }
